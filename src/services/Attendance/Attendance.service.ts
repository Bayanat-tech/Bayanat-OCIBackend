...
export class AttendanceService {
  ...
  private static async uploadImageIfNeeded(uuid: string): Promise<string | null> {
    try {
      ...
      const url = await S3Service.uploadFile(pendingData.image_buffer, key, 'image/jpeg');
  ...
  private static async getEmployeeImage(employeeId: string): Promise<string | null> {
    try {
      ...
      if (!imageUrl) {
        ...
        imageUrl = employeeFace ? await S3Service.getSignedUrl(employeeFace.s3_key) : null;
