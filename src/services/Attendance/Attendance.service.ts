...
export class AttendanceService {
  ...
  //   employeeId: string,
  //   action: "check-in" | "check-out"
  // ): Promise<{ status: string; timestamp: Date; employeeCode: string; employeeName: string }> {
  //   const today = new Date();
  //   today.setHours(0, 0, 0, 0);
  ...
  // Add this new method to your AttendanceService class
  static async markAttendanceWithLocation(
  employeeId: string,
  action: "check-in" | "check-out",
  locationData?: any
): Promise<{ status: string; timestamp: Date; employeeCode: string; employeeName: string }> {
  ...
  const eventType = action === "check-in" ? "check_in" : "check_out";
  ...
  if (locationData) {
    eventData.latitude = locationData.latitude;
    eventData.longitude = locationData.longitude;
    eventData.accuracy = locationData.accuracy;
    eventData.location_type = locationData.locationType;
    eventData.address = locationData.address;
    eventData.office_name = locationData.officeName;
  }

  // 🛠️ FIX: Create the event ONCE (not inside if/else)
  ...
  if (action === "check-in") {
    ...
    logger.info(`Employee ${employeeId} checked in at ${now} - Location: ${locationData?.locationType || 'unknown'}`);
    return {
      status: record.status,
      timestamp: now,
      employeeCode: employee.employee_code,
    ...
    logger.info(`Employee ${employeeId} checked out at ${now} - Location: ${locationData?.locationType || 'unknown'}`);
    return {
      status: record.status,
      timestamp: now,
      employeeCode: employee.employee_code,
