...
async function loadPrRegisterOldData(req: RequestWithUser, p: ReqParams, dispatchParam: string): Promise<ReportRow[]> {
...
// ─── Details report line model (grouped by Request No, item-wise) ─────────
...
function groupByRequest(rows: ReportRow[]): { groups: RequestGroup[]; grandAmount: number } {
  const byReq = new Map<string, RequestGroup>();
  const order: string[] = [];

  for (const r of rows) {
    const key = text(r.request_number);
    if (!byReq.has(key)) {
      byReq.set(key, {
        request_number: key,
        request_date: r.request_date,
        purch_status: text(r.purch_status),
        rows: [],
        amountTotal: 0,
      });
      order.push(key);
    }
    const g = byReq.get(key)!;
    g.rows.push(r);
    g.amountTotal += num(r.amount);
  }

  const groups = order.map((k) => byReq.get(k)!);
  const grandAmount = groups.reduce((s, g) => s + g.amountTotal, 0);
...
function buildDetailsExcelBuffer(rows: ReportRow[], loginId: string): Buffer {
  ...
  const COL_COUNT = 8; // Sr No, Item Code, Item Description, Rate, Qty, Currency, EX Rate, Amount
  ...
  rows_.push([cell(DETAILS_REPORT_TITLE, "title"), null, null, null, null, null, null, null]);
  merges.push({ s: { r: 0, c: 0 }, e: { r: 0, c: COL_COUNT - 1 } });

  rows_.push([
    cell(`Date: ${printDateTime}`, "meta"), null,
    cell(`User: ${loginId}`, "meta"), null,
    cell(`Report: ${REPORT_NAME}`, "meta"), null, null, null,
  ]);
  merges.push({ s: { r: 1, c: 0 }, e: { r: 1, c: 1 } });
  merges.push({ s: { r: 1, c: 2 }, e: { r: 1, c: 3 } });
  merges.push({ s: { r: 1, c: 4 }, e: { r: 1, c: 7 } });

  rows_.push([null, null, null, null, null, null, null, null]);

  rows_.push([
    cell("Sr No", "header"), cell("Item Code", "header"), cell("Item Description", "header"),
    cell("Rate", "header"), cell("Qty", "header"), cell("Currency", "header"),
    cell("EX Rate", "header"), cell("Amount", "header"),
  ]);

  groups.forEach((g) => {
    const rIdx = rows_.length;
    rows_.push([
      cell(`Request No. ${g.request_number}   ${dateText(g.request_date)}   ${g.purch_status}`, "groupHeader"),
    ...
    g.rows.forEach((r) => {
      rows_.push([
        cell(text(r.sr_no), "data"),
        cell(text(r.item_code), "data"),
        cell(text(r.item_desp), "data"),
        cell(num(r.item_rate), "dataNum3"),
        cell(num(r.item_qty), "dataNum3"),
        cell(text(r.curr_code), "data"),
        cell(num(r.currency_rate), "dataNum2"),
        cell(num(r.amount), "dataNum3"),
      ]);
    ...
    const dtRow = rows_.length;
    rows_.push([
      cell(`Total Amount for ${g.request_number}`, "groupTotal"), null, null, null, null, null, null,
      cell(g.amountTotal, "groupTotalNum"),
    ]);
    merges.push({ s: { r: dtRow, c: 0 }, e: { r: dtRow, c: 6 } });

    rows_.push([null, null, null, null, null, null, null, null]);
  });

  const gtRow = rows_.length;
  rows_.push([cell("Grand Total Amount", "grandTotal"), null, null, null, null, null, null, cell(grandAmount, "grandTotalNum")]);
  merges.push({ s: { r: gtRow, c: 0 }, e: { r: gtRow, c: 6 } });

  rows_.push([null, null, null, null, null, null, null, cell("Powered by Bayanat Technology", "footer")]);
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  const workbookXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <sheets><sheet name="PR Register Details" sheetId="1" r:id="rId1"/></sheets>
</workbook>`;
...
// ─── Route handlers (matches your router.post names) ───────────────────────

export const getPrRegisterOldSummaryReportHtml = async (req: RequestWithUser, res: Response): Promise<void> => {
  try {
    const params = extractParams(req);
    const rows = await loadPrRegisterOldData(req, params, "PR_REGISTER_OLD_SUMMARY_19082026");
    if (!rows.length) {
...
export const getPrRegisterOldSummaryReportExcel = async (req: RequestWithUser, res: Response): Promise<void> => {
  try {
    const params = extractParams(req);
    const rows = await loadPrRegisterOldData(req, params, "PR_REGISTER_OLD_SUMMARY_19082026");
    if (!rows.length) {
    ...
    res.setHeader("Content-Disposition", 'attachment; filename="PR_Register_Old_Summary_Report.xlsx"');
    res.end(buffer);
  } catch (error: any) {
    console.error("PR Register Old Summary Excel error:", error);
...
export const getPrRegisterOldDetailReportHtml = async (req: RequestWithUser, res: Response): Promise<void> => {
  try {
    const params = extractParams(req);
    const rows = await loadPrRegisterOldData(req, params, "PR_REGISTER_OLD_DETAILS_19082026");
    if (!rows.length) {
      res.status(200).json({ success: false, message: "No data found for the selected criteria." });
      return;
    }
...
export const getPrRegisterOldDetailReportExcel = async (req: RequestWithUser, res: Response): Promise<void> => {
  try {
    ...
    const buffer = buildDetailsExcelBuffer(rows, params.loginid);

    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", 'attachment; filename="PR_Register_Old_Details_Report.xlsx"');
    res.end(buffer);
  } catch (error: any) {
    console.error("PR Register Old Detail Excel error:", error);
    res.status(error.status || 500).json({ success: false, message: error.message || "Unable to generate Excel" });
  }
};