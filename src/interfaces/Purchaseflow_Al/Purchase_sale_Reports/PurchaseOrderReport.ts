...
async function loadPurchaseOrderData(req: RequestWithUser, p: ReqParams): Promise<ReportRow[]> {
  const conn = await getConn(req);
  try {
    const binds: any = {
      parameter: "PORPT_PO_19082026",
      loginid: p.loginid,
    ...
    const result = await conn.execute(
      `DECLARE
       ...
       BEGIN
         PROC_BUILD_DYNAMIC_SQL_PURCHASE_ORDER(
           :parameter, :loginid,
...
function buildHeader(rows: ReportRow[]): PoHeader {
  const h = rows[0] || {};
  return {
...
const REPORT_TITLE = "Purchase Order";
const REPORT_SUBTITLE = "PO Document";

/** PO-only layout CSS (shared header/footer/table CSS comes from report_common) */
const PO_EXTRA_CSS = `
  .doc-title-row {
    display: flex;
  ...
  table.data-table .right { text-align: right; font-variant-numeric: tabular-nums; }
  table.data-table .amount { font-weight: 600; color: #065f46; }
...
function renderPoBody(rows: ReportRow[], loginId: string): string {
  ...
  const bodyRows = rows
    ...
    <tr>
      <td>${i + 1}</td>
      <td>${escapeHtml(r.prod_code)} ${escapeHtml(r.prod_name)}${
        r.det_remarks ? ` — ${escapeHtml(r.det_remarks)}` : ""
      }</td>
  ...
  return `
    <div class="doc-title-row">
      <div>
        ...
        <div class="doc-sub">${escapeHtml(REPORT_SUBTITLE)} — ${escapeHtml(header.div_name)}</div>
        ${
          header.cancelled
    ...
    ${
      rows.length === 0
        ? `<div class="empty-lines">No line items found for this PO.</div>`
        : `
...
// ─── Excel builder (same raw-OOXML engine as PO Order Register) ───────────
...
function buildExcelBuffer(rows: ReportRow[], loginId: string): Buffer {
  ...
  const COL_COUNT = 7; // S.No, Product/Description, Unit, Qty, Unit Rate, Gross Value, VAT Type
  ...
  rows_.push([
    ...
    cell("Qty", "header"), cell("Unit Rate", "header"), cell("Gross Value", "header"), cell("VAT Type", "header"),
  ]);
  ...
  rows_.push([cell("Total (Inclusive VAT)", "grandTotal"), null, null, null, null, cell(totals.inclusiveVat, "grandTotalNum"), null]);
  ...
  // ── Style registration engine (identical to PO Order Register) ──
...
export const getPurchaseOrderReportHtml = async (req: RequestWithUser, res: Response): Promise<void> => {
  try {
    const params = extractParams(req);
    const rows = await loadPurchaseOrderData(req, params);
    if (!rows.length) {
      res.status(200).json({ success: false, message: "No data found for this PO." });
      return;
    }
    ...
    const bodyHtml = renderPoBody(rows, params.loginid);
    ...
    console.error("Purchase Order Report HTML error:", error);
...
export const getPurchaseOrderReportExcel = async (req: RequestWithUser, res: Response): Promise<void> => {
  try {
    const params = extractParams(req);
    const rows = await loadPurchaseOrderData(req, params);
    if (!rows.length) {
    ...
    console.error("Purchase Order Report Excel error:", error);
