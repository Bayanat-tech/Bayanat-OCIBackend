...
getEmployeesUnder: async (supervisor_empid: string) => {
  try {
    ...
    console.error(`Error fetching employees under empid ${supervisor_empid}:`, error.message);
    return [];
  }
},
  ...
  try {
    ...
    console.error(`Error fetching employees under empid ${employeeId}:`, error.message);
    return [];
  }
  },
...
BTvalidateLeave: async (params: {
  ...
  const bindParams = {
    ...
    p_RESULT:           { dir: oracledb.BIND_OUT, type: oracledb.STRING, maxSize: 4000 },
  };
...
try {
    ...
    // Parse "S$$$44.16" → S = Success, 44.16 = available balance
    ...
    return {
      ...
      message: isSuccess
        ...
        : `Validation Failed`,
      raw: rawResult, // optional, remove if not needed
    };

  } catch (error: any) {
    console.error("Error executing FN_HR_LEAVE_VALIDATION_V1:", error.message);
    return {
      result: null,
      success: false,
      isValid: false,
      error: "Leave validation could not be completed. Please try again later.",
    };
  }
},
