...
// /**
//  * Renders the outer page shell (logo, meta, style).
//  * `tableHtml` is the full <table> element (with thead/tbody/tfoot).
//  * `drillLevel` tells the JS which next endpoint to call on row click.
//  *   null  → no drill (last level)
//  *   "l3"  → clicking L2 row goes to /drilldown/l3
//  *   "l4"  → clicking L3 row goes to /drilldown/l4
//  *   "ac"  → clicking L4 row goes to /drilldown/ac
//  *   "detail" → clicking AC row goes to /drilldown/detail
//  */
...
// }): string {
//   const {
//     title, username, reportName, tableHtml,
//     drillLevel, companyCode, fromDate, toDate, divisionCode,
//   } = opts;
...
//   // Drill-down JS: posts to parent window (ReportDialogPage) with next level params
...
//       /* map drill level → field that identifies each data row */
//       var CODE_FIELD = {
//         l3:     "l2_code",
//         l4:     "l3_code",
//         ac:     "l4_code",
...
//             code:         code,
//             codeField:    CODE_FIELD[DRILL_LEVEL],
//           }, "*");
//         });
//       });
//     })();
...
//     .divider-thick { border-top: 2px solid #000; margin: 8px 0 4px; }
...
//     .meta-label { font-weight: 700; width: 64px; flex-shrink: 0; }
//     .drill-hint {
//       font-size: 10px;
//       color: #1a5f4a;
//       background: #f0f9f5;
...
// /**
//  * POST /drilldown/l2
//  * Body: { company_code, from_date, to_date, division_code?, l2_code[]? }
//  *
//  * SQL (first): L2-level trial balance.
//  * Row click → drills to L3 passing the clicked l2_code.
//  */
...
//         AND TR_AC_DETAIL.doc_date    >= :fromDate
//         AND TR_AC_DETAIL.doc_date    <  :toDate
...
//       <table>
//         <thead><tr>
//           <th style="width:70px">L2 Code</th>
//           <th>Account Name</th>
...
//       reportName:   "rpt_drilldown_l2_trialbalance",
//       tableHtml,
//       drillLevel:   "l3",
//       companyCode,
//       fromDate,
...
// /**
//  * POST /drilldown/l3
//  * Body: { company_code, from_date, to_date, division_code?, l2_code (single, from drill-down) }
//  *
//  * SQL (second): L3-level grouped under a specific L2 prefix.
//  * Row click → drills to L4 passing the clicked l3_code.
//  */
// export const getDrilldownL3 = async (req: RequestWithUser, res: Response): Promise<void> => {
//   let conn: oracledb.Connection | undefined;
//   try {
//     const { companyCode, fromDate, toDate, divisionCode } = parseCommon(req);
//     console.log("Drilldown L3 request body:", req.body);

//     // l2_code arrives either as selected codes array (direct use) or single string (from drill)
//     const l2Codes = parseCodeArray(req.body.l2_code);
//     // Build IN-list for L3's WHERE: substr(l3_code,1,1) IN (l2_codes)
//     const codeIn = l2Codes.length
//       ? l2Codes.map(c => `'${c.replace(/'/g, "''")}'`).join(",")
//       : "'All'";
...
//     `;
//     console.log("Drilldown L3 SQL:", sql);
...
//       <table>
//         <thead><tr>
//           <th style="width:80px">L3 Code</th>
//           <th>Account Name</th>
//           <th class="right">Opening</th>
//           <th class="right">Debit Amount</th>
//           <th class="right">Credit Amount</th>
...
//       title,
//       username:     req.user?.loginid ?? "",
//       reportName:   "rpt_drilldown_l3_trialbalance",
//       tableHtml,
//       drillLevel:   "l4",
//       companyCode,
//       fromDate,
...
// /**
//  * POST /drilldown/l4
//  * Body: { company_code, from_date, to_date, division_code?, l3_code (single, from drill-down) }
//  *
//  * SQL (third): L4-level grouped under a specific L3 prefix.
//  * Row click → drills to AC passing the clicked l4_code.
//  */
...
//     const codeIn  = l3Codes.length
//       ? l3Codes.map(c => `'${c.replace(/'/g, "''")}'`).join(",")
//       : "'All'";
...
//       <table>
//         <thead><tr>
//           <th style="width:90px">L4 Code</th>
//           <th>Account Name</th>
//           <th class="right">Opening</th>
//           <th class="right">Debit Amount</th>
//           <th class="right">Credit Amount</th>
...
//       title,
//       username:     req.user?.loginid ?? "",
//       reportName:   "rpt_drilldown_l4_trialbalance",
//       tableHtml,
//       drillLevel:   "ac",
//       companyCode,
//       fromDate,
...
// /**
//  * POST /drilldown/ac
//  * Body: { company_code, from_date, to_date, division_code?, l4_code (from drill), ac_code[]? }
//  *
//  * SQL (fourth): AC-level summary grouped by ac_code under a specific l4_code.
//  * Row click → drills to detail passing the clicked ac_code.
//  */
...
//     // Build IN-list clauses; "All" means no filter
//     const l4In = l4Codes.length
//       ? l4Codes.map(c => `'${c.replace(/'/g, "''")}'`).join(",")
//       : "'All'";
//     const acIn = acCodes.length
//       ? acCodes.map(c => `'${c.replace(/'/g, "''")}'`).join(",")
//       : "'All'";
...
//       title,
//       username:     req.user?.loginid ?? "",
//       reportName:   "rpt_drilldown_ac_trialbalance",
//       tableHtml,
//       drillLevel:   "detail",
//       companyCode,
//       fromDate,
...
// /**
//  * POST /drilldown/detail
//  * Body: { company_code, from_date, to_date, division_code?, ac_code (single, from drill) }
//  *
//  * SQL (fifth): Transaction-level ledger for a specific account, matching the
//  * PDF report: A/c Code | Type | Doc No | Doc Date | Chq No | Chq Date | Bank |
//  *              Debit | Credit | Balance
//  *
//  * Balance is a running total computed in-process (Oracle doesn't guarantee
//  * row order from GROUP BY, so we sort by doc_date, doc_no then accumulate).
//  * No drill further — this is the last level.
//  */
...
//             AND l.serial_no       = nvl(TR_AC_DETAIL.ref_doc_serial_no, 0))      lpo_otherremarks,
//         (SELECT ref_no
//            FROM tr_ac_header th
//           WHERE th.company_code   = TR_AC_DETAIL.company_code
//             AND th.doc_type       = nvl(TR_AC_DETAIL.doc_type, ' ')
//             AND th.doc_no         = nvl(TR_AC_DETAIL.doc_no, 0))                 ref_inv_no,
//         (SELECT ref_date
//            FROM tr_ac_header th
//           WHERE th.company_code   = TR_AC_DETAIL.company_code
//             AND th.doc_type       = nvl(TR_AC_DETAIL.doc_type, ' ')
//             AND th.doc_no         = nvl(TR_AC_DETAIL.doc_no, 0))                 ref_inv_dt,
//         TR_AC_DETAIL.div_code,
//         TR_AC_DETAIL.bank_ac_code,
...
//     // Group by ac_code so each account gets its own block with running balance
...
//       const opening   = amount(acRows[0]?.op_balance);  // always 0 per SQL; placeholder for future
...
//           <td class="num" style="font-weight:700; color:#1a5f4a">
//             Opening&nbsp;&nbsp;${escapeHtml(fmtNumber(opening))}
//           </td>
//           <td></td>
...
//       title,
//       username:     req.user?.loginid ?? "",
//       reportName:   "rpt_drilldown_detail_trailbalance",
//       tableHtml,
//       drillLevel:   null,   // last level — no further drill
//       companyCode,
//       fromDate,
//       toDate,
//       divisionCode,
