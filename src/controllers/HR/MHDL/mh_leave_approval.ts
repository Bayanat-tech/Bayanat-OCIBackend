  ...
  const { requestNumber, finalApproved } = await withTenantTransaction(
    async (connection: oracledb.Connection) => {
      ...
      if (exists) {
        ...
        await mhupdateLeaveApproval(dataWithUUID, connection);
      } else {
        console.log("Insert path with UUID:", uuid);
        await mhinsertLeaveApproval(dataWithUUID, connection);
...
async function recordExists(
  ...
  async function mhupdateLeaveApproval(data: TLeaveApproval, connection: any) {
...
async function mhinsertLeaveApproval(data: TLeaveApproval, connection: any) {
  console.log(
...
// === Express handler ===
export const mhupsertLeaveApprovalHandler = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
