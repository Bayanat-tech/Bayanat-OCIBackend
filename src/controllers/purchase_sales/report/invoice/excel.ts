...
/**
 * Sales Invoice – Excel exporter
 */
export function buildInvoiceExcelBuffer(rows: ReportRow[], loginId: string): Buffer {
  ...
  addRow(["INVOICE", ...Array(COL_COUNT - 1).fill("")], allStyle(styles.title));
  ...
  if (!header) {
    ...
    if (header.due_date) {
      addRow(["", "", "", `Due Date: ${header.due_date}`, "", ""], { 3: styles.value });
    }
    if (header.delivery_to) {
