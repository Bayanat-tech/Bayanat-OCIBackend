...
/**
 * Sales Invoice – HTML renderer
 * Matches sample SINVOICE layout (title INVOICE, Invoice No., totals, signatures).
 */
export function renderInvoiceHtml(rows: ReportRow[], loginId: string): string {
...
<body style="font-family:Arial,sans-serif;padding:40px;text-align:center;color:#666">
  <h2>No data found</h2>
  <p>Invoice not found for the given document number.</p>
</body></html>`;
  }

  const { totalQty, totalAmount, totalTax, overallDiscount, grandTotal } = calcTotals(
    lines,
...
<head>
  <meta charset="utf-8"/>
  <title>Invoice - ${escapeHtml(header.doc_no)}</title>
  <style>
...
<main class="sheet">
  ...
  <div class="meta-grid">
    ...
    <div class="meta-right">
      <table>
        ...
        ${
          header.due_date
            ? `<tr><td class="lbl">Due Date</td><td class="val">: ${escapeHtml(header.due_date)}</td></tr>`
            : ""
        }
        ${
          header.delivery_to
  ...
  <div class="totals-block">
    ...
    <div class="totals-right">
      <table>
        ...
        ${
          totalTax > 0
            ? `<tr><td class="lbl">Tax:</td><td class="val">${fmtNumber(totalTax)}</td></tr>`
            : ""
        }
        <tr>
  ...
  <div class="print-footer-block">
    ...
    <div class="report-footer">
      ...
      <span>Report: rpt_sales_invoice &nbsp;|&nbsp; Powered by Bayanat Technology</span>
    </div>
  </div>
</main>
</body>
