  ...
  await withConnection(res, async (connection) => {
    ...
    const result = await connection.execute(
       ...
       END;`,
      {
        ...
        p_driver_ref: value(job.driver_ref ?? job.DRIVER_REF),
        p_driver_remarks: value(job.driver_remarks ?? job.DRIVER_REMARKS),
        ...
        p_be_deposits: value(job.be_deposits ?? job.BE_DEPOSITS),
        p_be_dep_amount: numberValue(job.be_dep_amount ?? job.BE_DEP_AMOUNT),
        ...
        p_dec_no: value(job.dec_no ?? job.DEC_NO),
        P_dec_date: toDate(job.dec_date ?? job.DEC_DATE),
        p_delivered_on: toDate(job.delivered_on ?? job.DELIVERED_ON),
        p_picked_date: toDate(job.picked_date ?? job.PICKED_DATE),
        // p_clearance_date: toDate(job.clearance_date ?? job.CLEARANCE_DATE),
        p_letter_undertaking: toDate(job.letter_undertaking ?? job.LETTER_UNDERTAKING),
        p_contr_deposit: value(job.contr_deposit ?? job.CONTR_DEPOSIT),
        p_contr_deposit_amt: numberValue(job.contr_deposit_amt ?? job.CONTR_DEPOSIT_AMT),
        p_health_status: value(job.health_status ?? job.HEALTH_STATUS),
