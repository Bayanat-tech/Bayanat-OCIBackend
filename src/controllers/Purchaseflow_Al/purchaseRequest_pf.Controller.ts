...
export const createOrUpdatePurchaseRequestSequential = async (
  ...
  /*const purchaseRequest = {
      request_number,
      request_date: new Date(request_date), // Ensure it's a Date object
      description,
      project_code,
      company_code,
      created_by,
      ...
      service_request,
      last_action,
      created_at: new Date(created_at), // Ensure dates are converted correctly
      updated_at: new Date(updated_at), // Ensure dates are converted correctly
      flow_level_running,
      fa_uploaded,
      final_approved,
  
      // Mapping `items` into the required structure
      items: Array.isArray(items)
        ? items.map((item) => ({
            item_code: item.item_code,
            item_desp: item.item_desp,
            item_group_code: item.item_group_code,
            item_rate: parseFloat(item.item_rate), // Ensure it's a number
            p_uom: item.p_uom,
            l_uom: item.l_uom,
            upp: parseFloat(item.upp), // Ensure it's a number
            item_l_qty: parseFloat(item.item_l_qty), // Ensure it's a number
            item_p_qty: parseFloat(item.item_p_qty), // Ensure it's a number
            appr_upp: item.appr_upp ? parseFloat(item.appr_upp) : null,
            appr_item_l_qty: item.appr_item_l_qty ? parseFloat(item.appr_item_l_qty) : null,
            appr_item_p_qty: item.appr_item_p_qty ? parseFloat(item.appr_item_p_qty) : null,
            currency_rate: item.currency_rate ? parseFloat(item.currency_rate) : null,
            amount: parseFloat(item.amount), // Ensure it's a number
            company_code: item.company_code,
            updated_at: new Date(item.updated_at), // Ensure it's a Date object
            updated_by: item.updated_by,
            request_number: item.request_number,
            curr_code: item.curr_code,
            lcurr_amt: item.lcurr_amt ? parseFloat(item.lcurr_amt) : null,
            allocated_approved_quantity: parseFloat(item.allocated_approved_quantity),
            supplier: item.supplier,
            service_rm_flag: item.service_rm_flag,
            addl_item_desc: item.addl_item_desc,
            flow_level_running: item.flow_level_running,
            pr_amount: item.pr_amount ? parseFloat(item.pr_amount) : null,
            po_amount: item.po_amount ? parseFloat(item.po_amount) : null,
            month_budget: item.month_budget ? parseFloat(item.month_budget) : null,
            cost_code: item.cost_code,
            cost_name: item.cost_name,
          }))
        : [],
  
      // Mapping `Termscondition` into the required structure
      termconditions: Array.isArray(Termscondition)
        ? Termscondition.map((term) => ({
