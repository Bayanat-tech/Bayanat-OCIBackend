...
export const insSecRoleFunctionAccessUser = async (
    ...
    try {
        ...
        // NEW: role_id and company_code pulled from request body (top-level)
        const roleId = req.body?.role_id ?? null;
        const companyCode = req.body?.company_code ?? null;
        ...
        const roleAccessRows = rows.map((r: any) => ({
            ...
            USER_DT: r.user_dt ? new Date(r.user_dt) : new Date(),
            USERID: r.userid ?? null,
            CREATE_USER: r.create_user ?? null,
            CREATE_DATE: r.create_date ? new Date(r.create_date) : new Date()
        ...
        await connection.execute(
            ...
            BEGIN
                ...
                PROC_INS_SEC_ROLE_FUNC_ACCESS_USER(:p_data, :p_role_id, :p_company_code);
